    ...
    <motion.div
      ...
      className={`w-5 h-5 sm:w-6 sm:h-6 rounded-lg flex items-center justify-center shadow-sm relative overflow-hidden ${
        completed
          ...
          : "bg-[#1a120c] border border-orange-700/30"
      }`}
    >
      {completed && (
        <motion.div
      ...
      {completed ? (
        ...
        <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-orange-700/50" />
      )}
    </motion.div>
    <motion.div
      initial={{ opacity: 0, y: 5 }}
      ...
      transition={{ delay: index * 0.08 + 0.2 }}
      className={`text-[9px] sm:text-[10px] font-semibold ${
        completed ? "text-orange-400" : "text-orange-700/50"
      }`}
    >
      {time > 0 ? `${time}s` : "—"}
    </motion.div>
...
const PodiumCard = ({ rank, team, delay = 0 }) => {
  ...
  const rankConfig = {
    1: {
      ...
      shadow: "shadow-[0_20px_40px_rgba(255,117,24,0.35)]",
      icon: "👑",
      label: "Champion",
      height: "h-70 sm:h-100",
    },
    2: {
      ...
      shadow: "shadow-[0_15px_30px_rgba(168,85,247,0.3)]",
      icon: "🦇",
      label: "Runner Up",
      height: "h-70 sm:h-90",
    },
    3: {
      ...
      shadow: "shadow-[0_15px_30px_rgba(249,115,22,0.3)]",
      icon: "🎃",
      label: "Third Place",
      height: "h-70 sm:h-80",
    },
  ...
  return (
    <div ref={ref} className="flex flex-col items-end justify-end w-full sm:w-auto">
      <motion.div
        whileHover={{ scale: 1.05, y: -8 }}
        transition={{ type: "spring", stiffness: 300 }}
        className={`w-full sm:w-56 bg-[#0f0a06] rounded-t-3xl p-4 sm:p-5 ${config.shadow} border-2 border-orange-700/40 relative overflow-hidden ${config.height}`}
      >
        <motion.div
          animate={{ 
            scaleX: [1, 1.05, 1],
        ...
        <motion.div
          animate={{ rotate: [0, 5, -5, 0] }}
          transition={{ duration: 1, repeat: Infinity, repeatDelay: 0 }}
          className="text-4xl sm:text-5xl mb-2 sm:mb-3 text-center relative z-10 drop-shadow-[0_0_10px_rgba(255,165,0,0.5)]"
        >
          {config.icon}
        </motion.div>
        <div className="text-center mb-2 sm:mb-3">
        ...
        <div className="text-center mb-3 sm:mb-4 text-base sm:text-lg font-black text-orange-200"
...
const TeamListItem = ({ team, rank }) => (
  <motion.div
    ...
    className="bg-gradient-to-r from-[#0f0a06] to-[#17110b] rounded-2xl p-3 sm:p-4 shadow-md border-2 border-orange-700/30 hover:border-orange-500/50 transition-all max-w-2xl mx-auto relative overflow-hidden"
  >
    <motion.div
      className="absolute inset-0 bg-gradient-to-r from-orange-500/0 via-orange-500/5 to-purple-500/0"
      animate={{ x: ['-100%', '100%'] }}
      transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
    />
    ...
    <div className="flex items-start gap-3 sm:gap-4 relative z-10">
      <motion.div
        whileHover={{ rotate: 360, scale: 1.1 }}
        transition={{ duration: 0.6 }}
        className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-orange-500 to-purple-600 flex items-center justify-center text-white font-black text-lg sm:text-xl shadow-lg flex-shrink-0 relative overflow-hidden"
      >
        <motion.div
          animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
          transition={{ duration: 2, repeat: Infinity }}
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
          <div>
            <motion.div 
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              className="text-base sm:text-lg font-bold text-orange-200 mb-1 break-words"
              style={{fontFamily:"Creepster,sans-serif"}}
            >
              {team.name}
            </motion.div>
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-xs">
              <motion.span 
                whileHover={{ scale: 1.1 }}
                className="px-2 py-0.5 rounded-full bg-orange-900/40 text-orange-300 font-semibold whitespace-nowrap border border-orange-700/30"
              >
                {team.tasksCompleted}/4 Tasks
              </motion.span>
              {team.tasksCompleted === 4 && (
                <motion.span 
                  ...
                  animate={{ scale: 1 }}
                  whileHover={{ scale: 1.1 }}
                  className="px-2 py-0.5 rounded-full bg-orange-900/40 text-orange-300 font-semibold flex items-center gap-1 whitespace-nowrap border border-orange-700/30"
                >
                  <motion.svg 
                    animate={{ rotate: [0, 360] }}
                    transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
          ...
          <div className="text-left sm:text-right">
            <motion.div 
              whileHover={{ scale: 1.05 }}
              className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gradient-to-r from-orange-500 to-purple-600 text-white shadow-md relative overflow-hidden"
            >
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
...
export default function HalloweenLeaderboard() {
  const [list, setList] = useState([]);
  const [currentTeam, setCurrentTeam] = useState(null);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center relative overflow-hidden text-orange-100 p-3 sm:p-6" style={{background: 'radial-gradient(1200px 600px at 20% 10%, rgba(255,117,24,0.15), transparent 60%), radial-gradient(1000px 500px at 80% 20%, rgba(106,13,173,0.15), transparent 60%), linear-gradient(180deg, #0b0b0c 0%, #0f0a06 100%)'}}>
      ...
      <div className="absolute top-12 left-12 text-6xl pumpkin pointer-events-none opacity-40" aria-hidden>🎃</div>
      <div className="absolute top-20 right-20 text-5xl pumpkin pointer-events-none opacity-40" style={{ animationDelay: "0.5s" }} aria-hidden>🦇</div>
      <div className="absolute bottom-20 left-20 text-5xl pumpkin pointer-events-none opacity-40" style={{ animationDelay: "1s" }} aria-hidden>👻</div>
      <div className="absolute bottom-24 right-32 text-4xl pumpkin pointer-events-none opacity-40" style={{ animationDelay: "1.5s" }} aria-hidden>🕷️</div>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6 }}
        className="bg-[#0f0a06]/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border-2 border-orange-700/40 w-full max-w-5xl overflow-hidden relative z-10"
      >
        {/* Header Section */}
        <div className="relative bg-gradient-to-br from-orange-900/60 via-purple-900/60 to-fuchsia-900/60 px-4 sm:px-8 py-6 sm:py-10 overflow-hidden border-b-2 border-orange-700/40">
          <div className="absolute inset-0 opacity-10">
            <motion.div 
              animate={{ x: [0, 20, 0], y: [0, -20, 0], scale: [1, 1.1, 1] }}
              transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
              className="absolute top-0 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-orange-500 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" 
            />
            <motion.div 
              animate={{ x: [0, -20, 0], y: [0, 20, 0], scale: [1, 1.15, 1] }}
              transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
              className="absolute bottom-0 right-0 w-64 h-64 sm:w-96 sm:h-96 bg-purple-600 rounded-full blur-3xl translate-x-1/2 translate-y-1/2" 
            />
          </div>
          ...
          <div className="relative z-10 text-center">
            <motion.div
              ...
              transition={{ delay: 0.2, type: "spring", stiffness: 200 }}
              whileHover={{ rotate: 360, scale: 1.1 }}
              className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 mb-3 sm:mb-4 rounded-xl sm:rounded-2xl bg-orange-700/30 backdrop-blur-sm shadow-xl relative overflow-hidden border-2 border-orange-700/50"
            >
              <motion.div
                animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="absolute inset-0 bg-orange-500 rounded-xl sm:rounded-2xl"
              />
            ...
            <motion.h1
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3 }}
              className="text-3xl sm:text-4xl md:text-5xl font-black text-orange-200 mb-2 tracking-tight uppercase drop-shadow-[0_0_20px_rgba(255,117,24,0.5)]"
              style={{fontFamily:"Creepster,sans-serif", letterSpacing:"0.08em"}}
            >
              The Sensor Vault
            </motion.h1>
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.4 }}
              className="flex items-center justify-center gap-2 sm:gap-3 text-orange-300/90 mb-4 sm:mb-6 text-sm sm:text-base"
            >
              <motion.span whileHover={{ scale: 1.1 }} className="font-semibold">
                Synergy'25
              </motion.span>
              <motion.span 
                animate={{ scale: [1, 1.5, 1], opacity: [0.6, 1, 0.6] }}
                transition={{ duration: 2, repeat: Infinity }}
                className="w-1 h-1 rounded-full bg-orange-400/60" 
              />
              <motion.span whileHover={{ scale: 1.1 }} className="font-semibold">
                IIIT Bangalore
              </motion.span>
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 0.6 }}
          className="relative bg-gradient-to-r from-orange-600/80 to-purple-700/80 px-4 sm:px-6 py-3 sm:py-4 border-y-2 border-orange-700/40 overflow-hidden"
        >
          <motion.div
            animate={{ x: ['0%', '100%'] }}
            transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap text-xs sm:text-sm relative z-10">
            {currentTeam ? (
              <>
                <div className="flex items-center gap-1.5 sm:gap-2" style={{fontFamily:"Creepster,sans-serif"}}>
                  <motion.div
                    animate={{ scale: [1, 1.3, 1], opacity: [1, 0.5, 1] }}
                    transition={{ duration: 1.5, repeat: Infinity }}
                    className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-red-400 rounded-full shadow-lg"
                  />
                  <span className="font-semibold text-orange-100">LIVE</span>
                </div>
                <div className="h-3 sm:h-4 w-px bg-orange-300/30" />
                <span className="font-medium text-orange-200/80">Playing:</span>
                <motion.span 
                  key={currentTeam.name}
                  initial={{ scale: 0.8, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  className="text-sm sm:text-base font-black text-orange-100"
                >
                  {currentTeam.name}
                </motion.span>
                <div className="flex items-center gap-1">
                  <span className="text-[10px] sm:text-xs font-medium text-orange-200/70">Tasks:</span>
                  <motion.span 
                    key={`${currentTeam.tasksCompleted}-4`}
                    initial={{ scale: 1.2 }}
                    animate={{ scale: 1 }}
              ...
              <span className="font-medium text-orange-200/90">No team currently playing</span>
            )}
          </div>
        </motion.div>
          ...
          <div className="flex flex-col items-center mb-8 sm:mb-10">
            ...
            >
              <motion.div
                ...
                transition={{ duration: 2, repeat: Infinity, repeatDelay: 1 }}
                className="inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-amber-400 to-yellow-500 shadow-lg relative overflow-hidden border-2 border-orange-700/50"
              >
                <motion.div
                  animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
                  transition={{ duration: 2, repeat: Infinity }}
            ...
            <motion.h2 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.8 }}
              className="text-2xl sm:text-3xl font-black text-orange-200 mb-1"
              style={{fontFamily:"Creepster,sans-serif"}}
            >
              Top Performers
            </motion.h2>
            <motion.p 
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 0.9 }}
              className="text-xs sm:text-sm text-orange-400/80 font-medium text-center px-4"
            >
              Best times and completion rates
            </motion.p>
          </div>
          ...
          {rest.length > 0 && (
            <div className="flex items-center gap-3 sm:gap-4 my-8 sm:my-10">
              <motion.div 
                animate={{ scaleX: [0, 1] }}
                transition={{ duration: 1, delay: 1 }}
                className="flex-1 h-px bg-gradient-to-r from-transparent via-orange-600/50 to-transparent origin-left" 
              />
              <motion.div
                initial={{ scale: 0, rotate: -180 }}
                animate={{ scale: 1, rotate: 0 }}
                transition={{ delay: 1.2, type: "spring" }}
                whileHover={{ scale: 1.1, rotate: 360 }}
                className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-orange-900/60 to-purple-900/60 border-2 border-orange-700/40 relative overflow-hidden"
              >
                <motion.div
                  animate={{ x: ['-100%', '100%'] }}
                  transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                ...
                />
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-300 relative z-10" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
                </svg>
                <span className="text-xs sm:text-sm font-bold text-orange-200 uppercase tracking-wider relative z-10"
                  style={{fontFamily:"Creepster,sans-serif"}}>
                  All Teams
                </span>
              </motion.div>
              <motion.div 
                animate={{ scaleX: [0, 1] }}
                transition={{ duration: 1, delay: 1 }}
                className="flex-1 h-px bg-gradient-to-r from-transparent via-purple-600/50 to-transparent origin-right" 
              />
            </div>
          )}
        ...
        {/* Footer */}
        <div className="bg-gradient-to-r from-orange-900/30 to-purple-900/30 px-4 sm:px-8 py-4 sm:py-6 border-t-2 border-orange-700/40 relative overflow-hidden">
          <motion.div
            animate={{ x: ['0%', '100%'] }}
            transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            className="absolute inset-0 bg-gradient-to-r from-transparent via-orange-400/10 to-transparent"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 relative z-10">
            <div className="flex items-center gap-2">
              <motion.div 
                whileHover={{ rotate: 360, scale: 1.1 }}
                transition={{ duration: 0.6 }}
                className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-gradient-to-br from-orange-500 to-purple-600 flex items-center justify-center flex-shrink-0 relative overflow-hidden"
              >
                <motion.div
                  animate={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
                  transition={{ duration: 2, repeat: Infinity }}
              ...
              <div className="text-xs sm:text-sm text-center sm:text-left">
                <div className="text-orange-300/80 font-medium">Organized by</div>
                <div className="text-orange-200 font-black">Shivansh Shah, Aryan Sharma & Vedant Mundada</div>
              </div>
            </div>
            
            <div className="flex items-center gap-3 sm:gap-4 text-[10px] sm:text-xs text-orange-400/70">
              <div className="flex items-center gap-1">
                <motion.svg 
                  animate={{ rotate: 360 }}
                  transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
              ...
              <div className="h-3 w-px bg-orange-700/50" />
              <span className="font-medium">Synergy 2025</span>
            </div>
          </div>
        </div>
